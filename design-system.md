...
|---|---|---|
| A1 | `index.html` | `6bee4048c7a66e2a650d1cc127948d02bb8ef6a286774435ce5e1685e957e187` |
| A1 | `styles.css` | `6e0214d5a5bee58da478d0300120e839b99156f200f1edd546f9c73578e3160f` |
| A2 | `projects/index.html` | `a4ee8ce019fe455cd5ccf7555ba59caa56972c97a0edbf7387dedbbb3575aa5a` |
| A3 | `build-log/index.html` | `541d39897264bd8f7abe10c8a08055c50527abfe3deec75b1f19ed503c52f6fb` |
| A3 | `build-log/log.css` | `bc3fd9c2cacaaebde4038402644f4ac1988891aa438c6638ee9df31cdf89d296` |
| A4 | `exhibits/index.html` | `2f6b8737c870115d041eef768620d6f126acd243707876288b40802053907d30` |
...
**Re-pin (2026-09-26, Patrick: "recheck and update as needed"):** A2 was re-pinned again after the placeholder copy landed; no line numbers moved. A2, A3 and A4 were re-pinned again after the D8 fix, A4 once more after D2, and A2 and A4 after D6. A2, A3 and A4 were re-pinned once more after D1, D3, D4 and D9; D3 and D9 deleted lines, so A2 traces past line 16 moved up one and A3 traces past line 47 moved up one (past the old line 68, two). The earlier re-pins after D8, D2 and D6 changed one line in each file, so no line numbers moved then. A1, A2 and A3 are re-pinned to the files as committed with this change. The original extraction pins could not be recovered, because those working-tree versions were never committed. So every trace into these five files was re-derived by content against the current files, and the line numbers were updated. Traces to *Superseded (for reference)* hub values cite the last pre-D5 versions as `styles.css@1344aab` and `index.html@1344aab` (`git show 1344aab:styles.css`). A5, N1, R1, R2 and N2 match their pins (A4 was re-pinned after D8, D2 and D6, below). A2, A3 (`log.css`) and A4 were re-pinned after the D10–D12 roll-out. A1 (`index.html`) and A3 (`build-log/index.html`) were re-pinned again after their stylesheet links gained a `?v=p7a` version tag, so browsers fetch the P7a stylesheets instead of a cached copy; no line numbers moved. A1 (`index.html`, `styles.css`) was re-pinned again after the P7a wide-screen change; every hub trace into `index.html` below the masthead was re-derived by content (the section wrappers moved those lines). The earlier drift notes for A2 (the N1 card at `projects/index.html:169-186` and the N2 card at `:187-205`) are folded into this re-pin.
...
- **Section system (A2, A3, A4):** tokens `--garnet / --garnet-deep / --paper / --gold / --ink / --ink-soft / --rule / --mono / --s-*`. These are identical in `projects/index.html:11-24`, `build-log/log.css:8-21` and `exhibits/index.html:11-24`. Fluid `clamp()` sizes, weight 700 headings.
...
- **MUST** use `#6E1E28` as the single accent: links, and the brand color. [`styles.css:2`, `styles.css:40`, `projects/index.html:12`, `projects/index.html:51`, `build-log/log.css:9`, `build-log/log.css:47`, `exhibits/index.html:12`]. A4 calls garnet and paper "the house garnet and paper" (`exhibits/index.html:127`). It also describes a page that "abandons the house palette" as a departure (`exhibits/index.html:113`).
- **MUST** set all text in Switzer, loaded from Fontshare, with the fallback `ui-sans-serif, system-ui, sans-serif`. [`styles.css:25`, `index.html:9`, `projects/index.html:9,27`, `build-log/index.html:16`, `build-log/log.css:24`, `exhibits/index.html:9,27`]
...
- **MUST** use 1px solid hairlines as the only structural border. [`styles.css:84-85,150`, `projects/index.html:68,74`, `build-log/log.css:52,55`, `exhibits/index.html:64,68`]
- **MUST NOT** use `box-shadow`, gradients or rounded corners on containers. There are zero `box-shadow`, `gradient` or container `border-radius` declarations in A1–A4 (`rg`). The only radius is `50%` on 8px status dots (`projects/index.html:60,63`).
...
|---|---|---|---|
| `#E4E5E2` | `--bg` / `--paper` | Page background. Also the far end of every section-system `color-mix()`. | `styles.css:4,23`; `projects/index.html:14,28`; `build-log/log.css:11,25`; `exhibits/index.html:14,28` |
| `#6E1E28` | `--accent` / `--garnet` | **Interactive and brand.** Link text, focus ring, wordmark tile, selection background, uppercase label text inside panels, SVG primary strokes. | `styles.css:2,40,51`; `projects/index.html:12,34,42,51,78`; `build-log/log.css:9,31,39,47`; `exhibits/index.html:12,34,42,62,83` |

### 2b. Section system (A2–A4)

| Hex / value | Token | Role | Trace |
|---|---|---|---|
| `#41111A` | `--garnet-deep` | **Heading ink.** h1 and h2 text, and h2 link text. | `projects/index.html:13,48,83`; `build-log/log.css:10,45,59`; `exhibits/index.html:13,48,73` |
| `#221317` | `--ink` | **Body text.** Also the bold emphasis inside muted paragraphs. | `projects/index.html:16,29,86`; `build-log/log.css:13,26`; `exhibits/index.html:16,29` |
| `#CDA96A` | `--gold` | **Ordinal and directional marks:** wordmark letters, `ISSUE NO.` / `EXHIBIT` index labels, the `→` arrow in link rows, SVG secondary strokes, the left rule on the quoted brief, the "specced" status. It is never used for body text. | `projects/index.html:15,42,65,79,116`; `build-log/log.css:12,39,57,69`; `exhibits/index.html:15,42,55,72,75` |
| `#E2C48C` | (literal) | **Selection text** on a garnet selection background. | `projects/index.html:34`; `build-log/log.css:31`; `exhibits/index.html:34` |
| `color-mix(in oklch, var(--ink) 70%, var(--paper))` | `--ink-soft` | **Secondary text:** deks, descriptions, dates, legend, footnote paragraphs, the "parked" status. | `projects/index.html:17,57,84,85,92`; `build-log/log.css:14,76`; `exhibits/index.html:17,50,77,85` |
| `color-mix(in oklch, var(--ink) 55%, var(--paper))` | (inline) | **Metadata line:** the dates and the tech fingerprint under list items. Lighter than `--ink-soft`. | `build-log/log.css:65`; `exhibits/index.html:81` (A3 + A4, and A3 copies A4; since 2026-09-26 also the hub stamp as `--ink-meta`, `styles.css:8,115`) |
| `color-mix(in oklch, var(--garnet) 22%, var(--paper))` | `--rule` | **Hairline dividers** between list items, and the image border. | `projects/index.html:18,68,74,91`; `build-log/log.css:15,52,55`; `exhibits/index.html:18,64,68` |
| `#3F6B3A` | (literal) | **Status "running" (in use now).** Text and dot. | `projects/index.html:64,115`. Single file with 3 uses, so it meets the threshold, but only A2 has status. |
| `#B4402F` | `--ember` | **Removed 2026-09-26 (D3).** It was declared but never referenced. | `projects/index.html@4c7d4aa:16` |
...
- **P1 — MUST** use `#221317` (`--ink`) for body text. [`projects/index.html:16`, `build-log/log.css:13`, `exhibits/index.html:16`]
  *Superseded (for reference):* `#33353A`, hub body ink [`styles.css@1344aab:3,17`].
- **P2 — MUST** use `#41111A` (`--garnet-deep`) for h1 and h2. [`projects/index.html:48,83`, `build-log/log.css:45`, `exhibits/index.html:48,73`]
  *Superseded (for reference):* `#1A1B1E`, hub `--ink-strong` [`styles.css@1344aab:4,59,92,100,124`].
- **P3 — MUST** use `--ink-soft` (`color-mix(in oklch, var(--ink) 70%, var(--paper))`) for secondary text. [`projects/index.html:17`, `build-log/log.css:14`, `exhibits/index.html:17`]
  *Superseded (for reference):* `#61636A`, hub `--ink-muted` [`styles.css@1344aab:5,65,106,127,142`].
- **P4 — MUST** use `--rule` (`color-mix(in oklch, var(--garnet) 22%, var(--paper))`) for hairlines. [`projects/index.html:18`, `build-log/log.css:15`, `exhibits/index.html:18`]
  *Superseded (for reference):* `#D2D3D0`, hub `--line` [`styles.css@1344aab:6,78,140`].
- **P5 — MUST** color the "parked" status with `--ink-soft`, both the legend dot and the status text/dot. [`projects/index.html:66`]
  *Fixed 2026-09-26 (D4):* the A2 legend dot was a literal `#8a8a86`; it now uses `var(--ink-soft)` (`projects/index.html:117`).
- **Related observation (not a rule):** "specced" uses a plain gold dot in the legend (`projects/index.html:116`) but gold at `filter: brightness(0.75)` for the status text (`projects/index.html:65`). The darkening is presumably for legibility on paper. That reason is inferred, not stated in the source.
...
- **MUST** use the system mono stack for labels and metadata in the section system: `ui-monospace, 'SF Mono', SFMono-Regular, Menlo, Consolas, monospace` via `--mono`. [`projects/index.html:19`, `build-log/log.css:16`, `exhibits/index.html:19`] A1 has no monospace at all.
...
- The wordmark uses weight 700 on every section page (D1, fixed 2026-09-26).

### 3b. Scale

**Section system:** four fluid steps, identical in A2, A3 and A4 [`projects/index.html:20-23`, `build-log/log.css:17-20`, `exhibits/index.html:20-23`]:
...
- **h1:** weight 700, `letter-spacing: -0.02em`, `line-height: 1.08`, `text-wrap: balance`. [`projects/index.html:48`, `build-log/log.css:45`, `exhibits/index.html:48`]
- **h2:** weight 700, `letter-spacing: -0.01em`. [`projects/index.html:83`, `build-log/log.css:58`, `exhibits/index.html:73`]
- **Body:** `line-height: 1.62`, and `text-wrap: pretty` on paragraphs. [`projects/index.html:31,49`, `build-log/log.css:28,46`, `exhibits/index.html:31,49`]

- **P6 — MUST** use the four fluid `--s-*` steps above, with 700-weight h1 and h2. [`projects/index.html:20-23,48,83`, `build-log/log.css:17-20,45,58`, `exhibits/index.html:20-23,48,73`]
...
- **P7 — MUST** use a body `line-height` of `1.62`. [`projects/index.html:31`, `build-log/log.css:28`, `exhibits/index.html:31`]
  *Superseded (for reference):* `1.68` [`styles.css@1344aab:21`].
- **P7 — MUST** cap the page column at `max-width: 66ch` below `64rem` (wider screens: see P7a), and secondary paragraphs at `58ch`. [`projects/index.html:36,85`, `build-log/log.css:33`, `exhibits/index.html:36,77`]
  *Superseded (for reference):* `max-width: 660px` [`styles.css@1344aab:27`].
- **P7a — MUST** widen on large screens. *Added 2026-09-26 by the builder; approved by Patrick ("commit").* Prompted by Patrick's request (2026-09-26): "it does not fill the page when larger… that should not be the case". At `min-width: 64rem` the page column grows to `max-width: 90rem`, and sections flow into columns: list items become a wrapping grid (`repeat(auto-fill, minmax(22rem, 1fr))` for short items on the hub and Build Log, `minmax(28rem, 1fr)` for Projects and Exhibits, whose items carry a paragraph), each item topped by a `--rule` hairline in place of the list's top border and the item's bottom border, and short sibling sections sit side by side. Paragraphs keep the `66ch` cap. Below `64rem` the single 66ch column is unchanged. [`styles.css:158-197`, `index.html:31-106`, `projects/index.html:100`, `build-log/log.css:79-92`, `exhibits/index.html:87`]
...
**Rules:**
- **MUST** set index labels (`ISSUE NO. 4`, `EXHIBIT 01`) in mono, 600, `--s--1`, `line-height: 1`, `letter-spacing: 0.12em`, gold. The uppercase is in the copy itself, not produced by CSS. [`build-log/log.css:57` + `build-log/index.html:53`; `exhibits/index.html:72` + `exhibits/index.html:111`] (A3 + A4 only; A3 copies A4.)
...
- **MUST** separate metadata fields with ` · ` (middle dot). [`projects/index.html:131`, `exhibits/index.html:114`, `index.html:111`]
- **MUST** end link rows with `→`. In A3 and A4 the arrow is gold and nudges 4px on hover. [`build-log/log.css:69-70`, `exhibits/index.html:75-76`; A2 uses the plain `→` character in link text, `projects/index.html:134`]
- **MUST** sign section pages with a `.sig` line reading `— Patrick King, CMT`, set 2.2–2.4rem below the content. [`projects/index.html:100,292`; `build-log/log.css:77`, `build-log/index.html:90`; `exhibits/index.html:87,142`]
- **Section page titles end with a period** in all three section pages: "Projects." "Build log." "Exhibits." [`projects/index.html:111`, `build-log/index.html:27`, `exhibits/index.html:98`]. This is a copy pattern. Under the §10 contract a builder never writes titles anyway.

**Single-source patterns:**
- **Status chip:** mono 600, `letter-spacing: 0.1em`, uppercased by CSS, preceded by an 8px round dot in the status color. [`projects/index.html:62-66`]
- **Legend and dates:** mono 500, not uppercased. [`projects/index.html:56,84`]
...
**Figure captions:** CSS exists (`projects/index.html:90-92`), but the only `<figure>` is inside a commented-out template (`projects/index.html:267-286`). No rendered caption existed at extraction, so captions are **UNSPECIFIED** in practice. *Update 2026-09-24:* the N1 card now renders the template figure (`projects/index.html:181-184`). That is a single use, and it sets no rule. *Update 2026-09-26:* the N2 card repeats it (`projects/index.html:200-203`). Both are research-note cards, so it is still no rule for other cards.

---

## 5. Borders, corners, shadows, marks

- **MUST** separate list items with a hairline: a top border on the list container and a bottom border on each item. [`projects/index.html:68,74`; `build-log/log.css:52,55`; `exhibits/index.html:64,68`]
- **MUST** keep corners square. There is no `border-radius` on any box. The wordmark is a square tile: 40px, `aspect-ratio: 1`, garnet ground, gold "PK". [`projects/index.html:39-44`, `build-log/log.css:36-41`, `exhibits/index.html:39-44`]
...
- **Focus — MUST** on every page (research notes: see §11): `:focus-visible` gets `outline: 2px solid #6E1E28` with `outline-offset: 3px`. [`styles.css:50-53`] This is promoted from A1 (2026-09-21). A3 has had one since `1a64faf` (`build-log/log.css:48`). A2 and A4 got the same rule with D6 (`projects/index.html:51`, `exhibits/index.html:86`).
- **Project marks (single-source, 9 uses):** 52px inline SVGs (40px ≤560px). Garnet primary stroke and gold secondary stroke, both `stroke-width: 2` with round caps. Garnet fills for nodes. [`projects/index.html:77-80,96`]
...
**Section system (rem):**
- Page padding: `clamp(3rem, 8vh, 5.5rem)` top and bottom, `clamp(1.25rem, 5vw, 2rem)` at the sides. [`projects/index.html:36`, `build-log/log.css:33`, `exhibits/index.html:36`]
- Header row to title: `margin-bottom: 2.6rem`; title to intro: `1.2rem`. [same three files, lines 38/48, 35/45, 38/48]
- List block: `margin-top: 2.6rem`. [`projects/index.html:68`, `build-log/log.css:52`, `exhibits/index.html:64`]
- List item padding is `1.6rem 0 1.7rem` in `build-log/log.css:54` and `exhibits/index.html:67`, but `1.8rem 0 2rem` in `projects/index.html:73`. That difference is minor and the A2 rows are taller because they carry an icon column. It is not raised as a PICK.
- h2 margins: `0.45rem 0 0.2rem`. [`build-log/log.css:58`, `exhibits/index.html:73`]
- Paragraph rhythm: `margin-block: 1em`. [`projects/index.html:49`, `build-log/log.css:46`, `exhibits/index.html:49`]
- Row layout (A2 only): a grid of `64px 1fr`, gap `clamp(1rem, 3vw, 1.8rem)`, collapsing to one column at ≤560px. [`projects/index.html:69-75,94-98`]
...
- **P8 — MUST** frame every page the same way (research notes: see §11; the hub: see the exception below):
  ...
  - **Traces:** [`projects/index.html:106-109,292`, `build-log/index.html:22-25,90`, `exhibits/index.html:93-96,142`]
...
- *Superseded (for reference):* before D8 (2026-09-26), the section pages used `a { color: garnet }` with the browser-default underline [`projects/index.html@f821a1c:52`, `build-log/log.css@f821a1c:48`]. They now carry the P9 rule on the same lines [`projects/index.html:51`, `build-log/log.css:47`, `exhibits/index.html:86`]. The section-specific exceptions stay as observed: the back-link and h2 title links show no underline until hover, and the h2 hover offset is 4px [`projects/index.html:45-46`, `build-log/log.css:42-43,59-60`, `exhibits/index.html:45-46`].

**Motion:**
- **MUST** keep motion to short transitions (0.15–0.16s): hover color, the arrow nudge, the row tint. [`styles.css:45`, `build-log/log.css:69`, `exhibits/index.html:69,75`]
- **MUST** honor `prefers-reduced-motion`. [`styles.css:55-58`, `build-log/log.css:72-74`, `exhibits/index.html:86`] A4 since D2 was fixed (`exhibits/index.html:86`).
...
- **Dark theme:** none.
- **Role of `--ember` / `#B4402F`:** none; the token was removed (D3).
...
| ID | Defect | Trace |
|---|---|---|
| D1 | **Resolved 2026-09-26 (proposed after Patrick's "resume", pushed on his go).** The wordmark requested `font-weight: 800`, but pages load Switzer only up to 700. A3 was fixed in `1a64faf`; A2 and A4 now use 700 too. | `projects/index.html:43`, `exhibits/index.html:43`, `build-log/log.css:40` |
...
| D3 | **Resolved 2026-09-26 (proposed after Patrick's "resume", pushed on his go).** `--ember: #B4402F` was declared but never used (0 `var(--ember)` references). The line is removed; A2 traces past line 16 moved up one. | `projects/index.html@4c7d4aa:16` |
| D4 | **Resolved 2026-09-26 (proposed after Patrick's "resume", pushed on his go).** The "parked" legend dot used a literal `#8a8a86` instead of `--ink-soft` (the P5 rule). It now uses `var(--ink-soft)`. | `projects/index.html:117` vs `:66` |
...
| D8 | **Resolved 2026-09-26 (Patrick: "lets go to d8").** A2, A3 and A4 broke P9 (the tuned link underline is a MUST on every page), using `a { color: garnet }` with the browser's default underline. All three now carry A1's rule: 1px thickness, 3px offset, 35% garnet at rest going to full garnet on hover, and a 160ms fade. On A2, which had no transitions before this change, the fade is switched off under `prefers-reduced-motion` on the same line (A3 already had a guard; A4 got one with D2). The listed exceptions are unchanged: back-links and build-log title links have no underline until hover, and the exhibit rows are block links with no underline. | `projects/index.html:51`, `build-log/log.css:47`, `exhibits/index.html:86` |
| D9 | **Resolved 2026-09-26 (proposed after Patrick's "resume", pushed on his go).** Dead CSS in A3: `.dim` and `.dek` had no users in `build-log/index.html` after the `1a64faf` restructure. Both rules are removed and the stylesheet link tag is bumped to `?v=d9`; A3 traces past line 47 moved up one (past the old line 68, two). | `build-log/log.css@4c7d4aa:47,68` |
| D10 | **Resolved 2026-09-26 (Patrick: "roll out").** A2 broke P7a: one 66ch column at every width. At `64rem` and wider the page now grows to `90rem` and the project list wraps into a `minmax(28rem, 1fr)` grid (2 columns from 1024px to 1920px). Appended to the existing `.sig` line, so no line numbers moved. | `projects/index.html:36,100` |
| D11 | **Resolved 2026-09-26 (Patrick: "roll out").** A3 broke P7a: one 66ch column at every width. At `64rem` and wider the page now grows to `90rem` and the issue list wraps into a `minmax(22rem, 1fr)` grid (3 columns at 1920px, 2 at 1024px). Added as a block at the end of the stylesheet. | `build-log/log.css:33,79-92` |
| D12 | **Resolved 2026-09-26 (Patrick: "roll out").** A4 broke P7a: one 66ch column at every width. At `64rem` and wider the page now grows to `90rem`, the brief keeps the 66ch cap, and the exhibit list wraps into a `minmax(28rem, 1fr)` grid (2×2). Appended to the existing `.sig` line, so no line numbers moved. | `exhibits/index.html:36,87` |
...
4. Bracketed placeholders present in the corpus stay as placeholders until Patrick supplies the copy. No placeholders remain in the corpus as of 2026-09-26. The last ones, on the "research system" card (`projects/index.html:230-233`), were replaced with copy Patrick approved ("a"). The build-log DEK placeholder was removed by `1a64faf`. A builder must never fill them.
...
- **A card on `/projects/`,** in the A2 card pattern. [`projects/index.html:180`]
- **Sign notes "Patrick"** (Patrick, 2026-09-24). N1 complies: "Patrick · workbypk.com" [`waveminer/spend-map/index.html:238`]. Site pages keep their own signature, "— Patrick King, CMT" (`projects/index.html:292`). This rule applies to notes only.
